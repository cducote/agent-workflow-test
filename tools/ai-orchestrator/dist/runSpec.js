...
function parseModeFromInputs(eventName, event) {
    ...
    if (eventName === "issue_comment") {
        ...
        if (cmd === "implement" || cmd === "fix" || cmd === "plan")
            return cmd;
        return "plan";
    }
    if (eventName === "pull_request") {
        const labelName = event?.label?.name || "";
        if (labelName === "ai-implement")
            return "implement";
        if (labelName === "ai-fix")
            return "fix";
        return "plan";
...
function parsePrNumber(eventName, event) {
...
async function findMostRecentAIPlan(repository, prNumber) {
    try {
...
export async function buildRunSpec() {
    ...
    const source = eventName === "workflow_dispatch"
        ? "workflow_dispatch"
